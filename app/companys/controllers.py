...
class CompanyView(MethodView):
    ...
    def get(self, company_id):
        if self.current_user.is_stafff:
            ...
            if company_id:
                company = Company.query.filter_by(id=company_id, owner_id=self.current_user.id).first_or_404()
                result = company_form_schema.dump(company)
            else:
                company = Company.query.filter_by(owner_id=self.current_user.id).all()
                result = jsonify({'company': company_list_form_schema.dump(company)})
        return result
